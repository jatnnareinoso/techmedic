...
if (isset($_SESSION['correo_paciente'])) {
    ...
    $sqlPaciente = "SELECT * FROM paciente WHERE correo = '$correo_paciente'";
    $resultPaciente = $conn->query($sqlPaciente);

    if ($resultPaciente) {
        $paciente = $resultPaciente->fetch(PDO::FETCH_ASSOC);

        $id_paciente = $paciente['id_paciente'];
        $sqlHistoriasClinicas = "SELECT hc.*, d.nombre AS nombre_doctor, d.apellido AS apellido_doctor, e.nombre AS nombre_enfermedad
        ...
        $resultHistoriasClinicas = $conn->query($sqlHistoriasClinicas);

        if ($resultHistoriasClinicas) {
            $historiasClinicas = $resultHistoriasClinicas->fetchAll(PDO::FETCH_ASSOC);
        } else {
            $errorInfo = isset($conn) ? $conn->errorInfo() : null;
            echo "Error al obtener las historias clínicas: " . ($errorInfo ? $errorInfo[2] : "No se pudo obtener información del error");
        }
    } else {
        $errorInfo = isset($conn) ? $conn->errorInfo() : null;
        echo "Error al obtener la información del paciente: " . ($errorInfo ? $errorInfo[2] : "No se pudo obtener información del error");
    }
} else {
            ...
            <div id="content">
                ...
                <nav class="navbar navbar-expand navbar-light bg-white topbar mb-4 static-top shadow">
                    ...
                    <div class="container-fluid">
                        <h1 class="h3 mb-4 text-gray-800">Consultas del Paciente</h1>
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="card shadow mb-4">
                                    <div class="card-header py-3">
                                        <h6 class="m-0 font-weight-bold text-primary">Historias Clínicas del Paciente</h6>
                                    </div>
                                    <div class="card-body text-center">
                                        <?php if (isset($historiasClinicas) && count($historiasClinicas) > 0) : ?>
                                            <table class="table">
                                                <thead>
                                                    <tr>
                                                ...
                                                <tbody>
                                                    <?php foreach ($historiasClinicas as $historiaClinica) : ?>
                                                        <tr>
                                                            <td><?php echo $historiaClinica['id_historia_clinica']; ?></td>
                                                            <td><?php echo $historiaClinica['fecha_registro']; ?></td>
                                                            <td><?php echo $historiaClinica['nombre_doctor'] . ' ' . $historiaClinica['apellido_doctor']; ?></td>
                                                            <td><?php echo $historiaClinica['antecedentes']; ?></td>
                                                            <td><?php echo $historiaClinica['resultados']; ?></td>
                                                            <td><?php echo $historiaClinica['nombre_enfermedad']; ?></td>
                                                            <td><?php echo $historiaClinica['observacion']; ?></td>
                                                        </tr>
                                                    <?php endforeach; ?>
                                                </tbody>
                                            </table>
