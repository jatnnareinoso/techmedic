...
if (isset($_SESSION['correo_paciente'])) {
    ...
    if ($result) {
        ...
        echo "Error: " . ($errorInfo ? $errorInfo[2] : "No se pudo obtener información del paciente");
        exit;
    }

    $id_paciente = $paciente['id_paciente'];
    $sqlMensajes = "SELECT * FROM mensaje WHERE id_receptor = :id_paciente ORDER BY fecha_envio DESC";
    $stmtMensajes = $conn->prepare($sqlMensajes);
    $stmtMensajes->bindParam(':id_paciente', $id_paciente, PDO::PARAM_INT);
                        ...
                        <div class="card">
                            ...
                            if ($mensajes) {
                                foreach ($mensajes as $mensaje) {
                                    ...
                                    $sqlDoctor = "SELECT nombre, apellido FROM doctor WHERE id_doctor = :id_doctor";
                                    $stmtDoctor = $conn->prepare($sqlDoctor);
                                    $stmtDoctor->bindParam(':id_doctor', $id_emisor, PDO::PARAM_INT);
                                    $stmtDoctor->execute();
                                    $doctor = $stmtDoctor->fetch(PDO::FETCH_ASSOC);

                                    echo '<p><strong>De: </strong>' . $doctor['nombre'] . ' ' . $doctor['apellido'] . '</p>';
                                    echo '<p><strong>Fecha: </strong>' . $mensaje['fecha_envio'] . '</p>';
                                    echo '<p><strong>Contenido: </strong>' . $mensaje['contenido'] . '</p>';
                                    echo '</div>';
                                }
                            } else {
