...
if (isset($_SESSION['correo_doctor'])) {
    ...
    if ($result) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            // Insertar el nuevo historial clínico en la base de datos
            $sqlInsertHistorial = "INSERT INTO historia_clinica (id_paciente, id_doctor, fecha_registro, antecedentes, resultados, id_enfermedad, observacion) 
                                 VALUES ('$id_paciente', '{$doctor['id_doctor']}', CURDATE(), '$antecedentes', '$resultados', '$id_enfermedad', '$observacion')";

            $resultInsertHistorial = $conn->exec($sqlInsertHistorial);

            if ($resultInsertHistorial) {
                // Redirigir a la página de historial clínico después de agregar uno nuevo
                header("Location: historial_clinico.php");
                exit;
            } else {
                // Manejar el caso en que ocurra un error al insertar el historial clínico
                $errorInfoInsertHistorial = isset($conn) ? $conn->errorInfo() : null;
                echo "Error: " . ($errorInfoInsertHistorial ? $errorInfoInsertHistorial[2] : "No se pudo insertar el historial clínico");
            }
        }
    } else {
        // Manejar el caso en que no se encuentre la información del doctor
...
$queryEnfermedades = "SELECT id_enfermedad, nombre FROM enfermedad";
$resultEnfermedades = $conn->query($queryEnfermedades);

// Verificar si las consultas fueron exitosas
if ($resultPacientes && $resultEnfermedades) {
    // Obtener los datos de los pacientes y enfermedades
    $pacientes = $resultPacientes->fetchAll(PDO::FETCH_ASSOC);
    $enfermedades = $resultEnfermedades->fetchAll(PDO::FETCH_ASSOC);
} else {
    // Manejar el caso en que la consulta falle
    echo "Error en la consulta de pacientes o enfermedades: " . $conn->errorInfo()[2];
}
                    ...
                    <div id="wrapper">
                        <div id="content">
                            ...
                            <div class="container-fluid">
                                <h1 class="h3 mb-4 text-gray-800">Crear Historial Clínico</h1>
                                <form method="post" action="crear_historial_clinico.php">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" name="antecedentes" required></textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="resultados">Resultados:</label>
                                        <textarea class="form-control" name="resultados" required></textarea>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <button type="submit" class="btn btn-primary">Crear Historial Clínico</button>
                                </form>
                            </div>
                        </div>
                    </div> 
